...
public class ConCreation {
    ...
    public DetteSimplifiee enregistrerRedevable(JTable table,JTextField mailRedevable, JTextField nomRedevable,JTextField libelle, JTextField montant, JTextField infoComplementaire, JTextField actionentreprendre,JTextField actioneffectuee, JComboBox listeAgent) throws  NumberFormatException, MontantException, VideException{
        ...
        if(sum == detSimp.getMontant()){
            daocreation.CreationRedevable(mailRedevable.getText(), nomRedevable.getText(), echeances, libelle.getText(), montant.getText(), infoComplementaire.getText(), actionentreprendre.getText(), actioneffectuee.getText(),  listeAgent.getSelectedItem().toString());
    ...
    public void nombreDesEcheances(JTextField montant, JComboBox nbEcheances, JTable listeEcheances){
        if (nbEcheances.getSelectedItem()!=null){
        if(!nbEcheances.getSelectedItem().equals("")){
            int  nb = Integer.parseInt((String)nbEcheances.getSelectedItem());
            ...
            if (montan != null && montan.length() > 0) {
                int mont= Integer.parseInt(montan);
                if (mont != 0){
                    resultat=mont/nb;
            ...
            int mont1 = Integer.parseInt(montan);
            model.addRow(new Object[]{
                    "Deadline"+nb,

                    " ", 
                    String.valueOf(mont1-(nb-1)*resultat)
                });
        }
        }
    }
